...
// Same failure mode, one step worse: og:image is an absolute URL, so the dead-
// reference check above cannot see it, and every page in every locale points at
// this one file. It shipped 404ing once already. Unlike logo.png it is not
// generated by the build — it is a committed Chromium screenshot (see
// scripts/gen-og.mjs) — so a bad merge or a stray clean can delete it and no
